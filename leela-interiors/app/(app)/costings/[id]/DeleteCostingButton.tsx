...
export function DeleteCostingButton({ costingId }: { costingId: string }) {
  const router = useRouter()
  const [deleting, setDeleting] = useState(false)
  const [error, setError] = useState<string | null>(null)

  async function handleDelete() {
    if (!window.confirm('Delete this costing? This cannot be undone.')) return

    setDeleting(true)
    setError(null)

    const res = await fetch(`/api/costings/${costingId}`, { method: 'DELETE' })
